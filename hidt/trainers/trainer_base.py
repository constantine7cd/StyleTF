...
class TrainerBase(nn.Module):
    ...
    def _init_models(self):
        ...
        for model_name, model_config in self.params['models'].items():
            ...
            setattr(self,
                    model_name,
                    getattr(networks, architecture)(model_config).cpu()
                    )
    ...
    def encode(self, data):
        if self.params['train_mode'] == 'DataParallel':
            ...
            return nn.parallel.data_parallel(self.gen, data,
                                             device_ids=list(
                                                 range(bs)) if bs < torch.cpu.device_count() else None,
    ...
    def _unpack_data(self, data):
        ...
        for map_name in data['a']:
            x_a[map_name] = data['a'][map_name].cpu()
            x_b[map_name] = data['b'][map_name].cpu()
        return x_a, x_b
